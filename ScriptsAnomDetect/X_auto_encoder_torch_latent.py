...
# Slight modification to get the latent space representation after training
...
class InnerAutoencoder(nn.Module):
    ...
    def latent(self, x):
        # Return the latent state representation
        x = self.encoder(x)
        return x
...
class AutoEncoder(BaseDetector):
    ...
    def latent_space(self, X):
        """Retrieve the latent space representation from trained model.

        Parameters
        ----------
        X : numpy array of shape (n_samples, n_features)
            The input samples to compress. Sparse matrices are accepted only
            if they are supported by the base estimator.

        Returns
        -------
        latent_space : numpy array of shape (n_samples, latent_size)
            The latent space representation of the input samples.
        """
        check_is_fitted(self, ['model', 'best_model_dict'])
        X = check_array(X)
        ...
        # construct the vector for holding the latent space, shape(batch_size, latent_size)
        latent_space = np.zeros([X.shape[0], self.hidden_neurons[-1]])
        with torch.no_grad():
            for data, data_idx in dataloader:
                data_cuda = data.to(self.device).float()
                latent_space[data_idx] = self.model.latent((data_cuda).cpu()).numpy()
